...
class _FullMaketState extends State<FullMaketView> {
  ...
  @override Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      const SizedBox(height: 6), const Text('Pilih type rumah, putar tampilan, zoom, dan sembunyikan atap untuk melihat interior.'),
      ...
      Wrap(spacing:8,runSpacing:8,children:[
        ...
        OutlinedButton.icon(onPressed:()=>setState((){yaw=-.55;pitch=.45;zoom=1;}),icon:const Icon(Icons.threed_rotation),label:const Text('Reset 3D')),
        OutlinedButton.icon(onPressed:()=>setState(()=>zoom=(zoom+.2).clamp(.65,2.5)),icon:const Icon(Icons.zoom_in),label:const Text('Zoom +')),
        OutlinedButton.icon(onPressed:()=>setState(()=>zoom=(zoom-.2).clamp(.65,2.5)),icon:const Icon(Icons.zoom_out),label:const Text('Zoom −')),
      ]),
      const SizedBox(height:16), Text('Atur ruangan',style:Theme.of(context).textTheme.titleLarge),const SizedBox(height:6),
      ...rooms.map((r)=>Card(child:ListTile(title:Text(r.name),subtitle:Text('X ${r.x.toStringAsFixed(1)} m • Y ${r.y.toStringAsFixed(1)} m'),trailing:Wrap(children:[
        IconButton(icon:const Icon(Icons.arrow_back),onPressed:()=>setState(()=>r.x=math.max(0,r.x-.25))),IconButton(icon:const Icon(Icons.arrow_forward),onPressed:()=>setState(()=>r.x=math.min(6-r.w,r.x+.25))),IconButton(icon:const Icon(Icons.arrow_upward),onPressed:()=>setState(()=>r.y=math.max(0,r.y-.25))),IconButton(icon:const Icon(Icons.arrow_downward),onPressed:()=>setState(()=>r.y=math.min(6-r.h,r.y+.25))),
      ]))),
    ]);
  }
...
class _ThumbPainter extends CustomPainter {
  final String type; final bool roof; _ThumbPainter({required this.type,required this.roof});
  @override void paint(Canvas c,Size s){final p=Paint()..style=PaintingStyle.fill..color=Colors.grey.shade300;final b=Rect.fromLTWH(s.width*.18,s.height*.38,s.width*.64,s.height*.45);c.drawRect(b,p);p.color=Colors.grey.shade500;if(roof){final path=Path()..moveTo(s.width*.12,s.height*.4)..lineTo(s.width*.5,s.height*.12)..lineTo(s.width*.88,s.height*.4)..close();c.drawPath(path,p);}p.color=Colors.grey.shade700;c.drawRect(Rect.fromLTWH(s.width*.44,s.height*.55,s.width*.12,s.height*.28),p);}
  @override bool shouldRepaint(covariant _ThumbPainter o)=>o.type!=type||o.roof!=roof;
}
class _House3DPainter extends CustomPainter {
  ...
  Offset project(double x,double y,double z,Size s){final cy=math.cos(yaw),sy=math.sin(yaw),cp=math.cos(pitch),sp=math.sin(pitch);final X=x*cy-y*sy,Y=x*sy+y*cy;final yy=Y*cp-z*sp,zz=Y*sp+z*cp,scale=math.min(s.width,s.height)*.065*zoom;return Offset(s.width/2+X*scale,s.height*.63-yy*scale-zz*scale);}
  @override void paint(Canvas c,Size s){final h=3.0,p=Paint()..style=PaintingStyle.fill;final pts=[project(0,0,0,s),project(length,0,0,s),project(length,width,0,s),project(0,width,0,s),project(0,0,h,s),project(length,0,h,s),project(length,width,h,s),project(0,width,h,s)];
    p.color=Colors.grey.shade300;c.drawPath(Path()..addPolygon([pts[0],pts[1],pts[5],pts[4]],true),p);p.color=Colors.grey.shade200;c.drawPath(Path()..addPolygon([pts[1],pts[2],pts[6],pts[5]],true),p);p.color=Colors.grey.shade400;c.drawPath(Path()..addPolygon([pts[4],pts[5],pts[6],pts[7]],true),p);
    final edge=Paint()..style=PaintingStyle.stroke..strokeWidth=1.6..color=Colors.black87;for(final pair in [[0,1],[1,2],[2,3],[3,0],[4,5],[5,6],[6,7],[7,4],[0,4],[1,5],[2,6],[3,7]])c.drawLine(pts[pair[0]],pts[pair[1]],edge);
    if(roofVisible){final z=h+1.25,r1=project(0,0,z,s),r2=project(length,0,z,s),r3=project(length,width,z,s),r4=project(0,width,z,s);p.color=type=='Limas'?Colors.brown.shade400:Colors.brown.shade300;c.drawPath(Path()..addPolygon([pts[4],pts[5],r2,r1],true),p);c.drawPath(Path()..addPolygon([pts[5],pts[6],r3,r2],true),p);c.drawPath(Path()..addPolygon([pts[6],pts[7],r4,r3],true),p);c.drawPath(Path()..addPolygon([pts[7],pts[4],r1,r4],true),p);}
