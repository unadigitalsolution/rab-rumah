...
class _AppState extends State<RabRumahApp> {
  ...
  @override Widget build(BuildContext context){
    ...
    return MaterialApp(debugShowCheckedModeBanner:false,theme:ThemeData(useMaterial3:true,colorSchemeSeed:Colors.indigo),home:Scaffold(
      ...
      drawer:AppDrawer(page:page,onNav:nav),body:pages[page],
...
class AppDrawer extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>Drawer(child:ListView(children:[
    ...
    _item(context,0,'Beranda',Icons.home),_item(context,1,'Buat RAB',Icons.add_box),_item(context,2,'Proyek Saya',Icons.folder),
    _item(context,3,'Material',Icons.inventory_2),_item(context,4,'Galeri',Icons.image),_item(context,5,'Laporan',Icons.assessment),
    _item(context,6,'Pengaturan',Icons.settings),_item(context,7,'Kalkulator',Icons.calculate),_item(context,8,'Kwitansi',Icons.receipt_long),
    const Padding(padding:EdgeInsets.all(16),child:Text('Offline-first • SPK mobile')),
  ]));
  Widget _item(BuildContext c,int i,String t,IconData icon)=>ListTile(selected:page==i,leading:Icon(icon),title:Text(t),onTap:(){Navigator.pop(c);onNav(i);});
}
...
class HomePage extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>ListView(padding:const EdgeInsets.all(16),children:[
    ...
    ]))),const SizedBox(height:12),Row(children:[Expanded(child:StatCard('Proyek','${projects.length}',Icons.folder)),Expanded(child:StatCard('RAB','${projects.length}',Icons.receipt_long))]),
...
class _EditorState extends State<EditorPage>{
  ...
  @override Widget build(BuildContext context){final r=RabEngine.calculate(input());final s=JsonMap.from(r['summary']);return ListView(padding:const EdgeInsets.all(16),children:[
    ...
    if(tab==0)...[TextField(controller:name,decoration:const InputDecoration(labelText:'Nama proyek',border:OutlineInputBorder())),const SizedBox(height:10),TextField(controller:owner,decoration:const InputDecoration(labelText:'Pemilik',border:OutlineInputBorder())),const SizedBox(height:10),TextField(controller:location,decoration:const InputDecoration(labelText:'Lokasi',border:OutlineInputBorder()))]
    else if(tab==1)...[Row(children:[Expanded(child:numBox('Panjang (m)',length,(v)=>setState(()=>length=v))),const SizedBox(width:8),Expanded(child:numBox('Lebar (m)',width,(v)=>setState(()=>width=v)))]),const SizedBox(height:10),Row(children:[Expanded(child:numBox('Tinggi dinding',height,(v)=>setState(()=>height=v))),const SizedBox(width:8),Expanded(child:numBox('Overhang',overhang,(v)=>setState(()=>overhang=v)))]),const SizedBox(height:10),DropdownButtonFormField<int>(value:floors,decoration:const InputDecoration(labelText:'Lantai',border:OutlineInputBorder()),items:[1,2,3].map((v)=>DropdownMenuItem(value:v,child:Text('$v lantai'))).toList(),onChanged:(v)=>setState(()=>floors=v??1)),const SizedBox(height:10),Card(child:ListTile(title:const Text('Luas bangunan'),trailing:Text('${(r['geometry']['floorArea'] as num).toStringAsFixed(2)} m²')))]
    else if(tab==2)...[selectBox('Dinding',wall,['lightbrick','brick','batako'],(v)=>setState(()=>wall=v)),selectBox('Lantai',floor,['tile','granite'],(v)=>setState(()=>floor=v)),selectBox('Atap',roof,['genteng','metal'],(v)=>setState(()=>roof=v)),selectBox('Plafon',ceiling,['gypsum','grc'],(v)=>setState(()=>ceiling=v)),Row(children:[Expanded(child:numBox('Pintu',doors.toDouble(),(v)=>setState(()=>doors=v.round()))),const SizedBox(width:8),Expanded(child:numBox('Jendela',windows.toDouble(),(v)=>setState(()=>windows=v.round())))])]
    else if(tab==3)...[SummaryTable(summary:s),Text('Komponen RAB: ${(r['items'] as List).length} item')]
    else...[MaketView(r:r,topView:topView,onTop:()=>setState(()=>topView=!topView))],
    const SizedBox(height:20),Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[if(tab>0)OutlinedButton(onPressed:()=>setState(()=>tab--),child:const Text('Kembali')) else const SizedBox(),FilledButton(onPressed:(){if(tab<4){setState(()=>tab++);}else{finish();}},child:Text(tab==4?'Simpan Proyek':'Lanjut'))]),
  ];}
}
